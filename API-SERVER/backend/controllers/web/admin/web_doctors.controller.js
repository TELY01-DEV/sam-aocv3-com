    ...
    if(req.body._id !== 'null'){
        ...
        .then(async data => {
            if (req.files && req.files.length) {
                    ProfileImageURL = await configure.fsUpload(data._id , 'doctorProfile',  req.files[0]);
                    doctor.userProfile = ProfileImageURL;
                    Doctors.findOneAndUpdate({_id :req.body._id},{$set :{profile_picture :ProfileImageURL}}).then((data,err) => {
                        console.log(err);
                    })
            }
            if (!data) {
                res.status(204).send(configure.apiResp(false, 204, {},  "Doctors data not found !.",req.headers.isencryption));
        ...
        .then(async data => {
           ...
           let ProfileImageURL = await configure.fsUpload(data._id, 'doctorProfile',  req.files[0]);
            doctor.userProfile = ProfileImageURL;
            Doctors.findOneAndUpdate({_id :data._id},{$set :{profile_picture :ProfileImageURL}}).then((data,err) => {
                console.log(err);
            })         
        }
              var response = {};
