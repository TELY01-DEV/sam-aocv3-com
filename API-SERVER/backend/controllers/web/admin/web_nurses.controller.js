...
exports.getNursesList = (req, res) => {
    ...
    .catch(err => {
        res.status(500).send(configure.apiResp(false, 500, {}, err.message || "Some error occurred while retrieving Nurses.",req.headers.isencryption));
    });
};

exports.updateNurseDetail = (req, res) => {
    var response = {};
    if(req.body._id !== 'null'){
        ...
        Nurses.findOneAndUpdate({_id: req.body._id}, { $set: nurse })
        .then(async data => {
            if (req.files && req.files.length) {
                    ProfileImageURL = await configure.fsUpload(data._id, 'nurseProfile',  req.files[0]);
                    nurse.nurseProfile = ProfileImageURL;
                    Nurses.findOneAndUpdate({_id :req.body._id},{$set :{profile_picture :ProfileImageURL}}).then((data,err) => {
                        console.log(err);
                    })
                // let user_image =  configure.fsUpload(data._id, 'nurseProfile',);
            }
            response['data'] = data;
        ...
        .catch(err => {
            res.status(500).send(configure.apiResp(false, 500, {}, err.message || "Some error occurred while updating Nurses.",req.headers.isencryption));
        });
    }else{
        ...
        .save(nurse)
        .then(async data => {
            // if (req.files && req.files.length) configure.fsUpload(data._id, 'nurseProfile', req.files[0]);
            if(req.files && req.files.length >0 ){
                ProfileImageURL = await configure.fsUpload(data._id, 'nurseProfile',  req.files[0]);
                nurse.nurseProfile = ProfileImageURL;
                Nurses.findOneAndUpdate({_id :data._id},{$set :{profile_picture :ProfileImageURL}}).then((data,err) => {
                    // console.log(data[0].nurseProfile);
                })
            }
            var response = {};
            response['success'] = true;
        ...
        .catch(err => {
            res.status(500).send(configure.apiResp(false, 500, {}, err.message || "Some error occurred while creating Nurses.",req.headers.isencryption));
        });
    }
};